...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        object_1__condition = Q(
            order__updated__lte=F('order__created') + object_1__time_delta)

        object_2__condition = Q(
            order__updated__gt=F('order__created') + object_1__time_delta) & \
                              Q(order__updated__lte=F(
                                  'order__created') + object_2__time_delta)

        object_expired__condition = Q(
            order__updated__gt=F('order__created') + object_2__time_delta)

        object_1__order = When(object_1__condition, then=OBJECT_1)
        object_2__order = When(object_2__condition, then=OBJECT_2)
        object_expired__order = When(object_expired__condition,
                                     then=OBJECT_EXPIRED)

        object_1__price = When(object_1__condition,
                               then=F('product__price') * F(
                                   'quantity') * object_1__discount)
        object_2__price = When(object_2__condition,
                               then=F('product__price') * F(
                                   'quantity') * -object_2__discount)
        object_expired__price = When(object_expired__condition,
                                     then=F('product__price') * F(
                                         'quantity') * object_expired__discount
                                     )
        ...
        for orderitem in test_orders:
            t_list.add_row([f'{orderitem.object_order} заказ №{orderitem.pk:}',
